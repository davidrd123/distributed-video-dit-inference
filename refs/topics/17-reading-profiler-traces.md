...
- **Prefer CUDA events for timing GPU work**: if you need per-stage/per-chunk GPU durations (e.g., Stage 0 decode time, Stage 1 Phase B time), use CUDA events (`torch.cuda.Event(enable_timing=True)`) so you can measure without repeatedly synchronizing the whole device. This avoids perturbing overlap schedules and reduces “profiling lies.” See: PP bringup guidance in `scope-drd/notes/FA4/h200/tp/pp0-bringup-runbook.md` and the PP1 overlap work in `scope-drd/notes/FA4/h200/tp/landscape.md` (measurement tightened by replacing repeated `cuda.synchronize()` with events).
