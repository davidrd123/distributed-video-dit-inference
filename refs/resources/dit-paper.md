...
- **Pipeline partitioning**: DiT-style generators are uniform stacks of blocks (no U-Net skip connections, no resolution changes mid-network). In Scope’s Wan2.1 deployment, the generator is **40 blocks (dim=5120, 40 heads)**, which is why both TP sharding and block-wise PP partitioning are straightforward. (See: `scope-drd/notes/FA4/h200/tp/explainers/01-why-two-gpus.md`; `scope-drd/notes/FA4/h200/tp/explainers/04-tp-math.md`.)
...
- **adaLN conditioning couples timestep to every block**: Treat conditioning as stage-global input — TP v0 already broadcasts `conditioning_embeds` every chunk, and the PP plan keeps `conditioning_embeds` as a required per-chunk tensor in `PPEnvelopeV1` so every stage can run its blocks without hidden dependencies. (See: `scope-drd/notes/FA4/h200/tp/explainers/03-broadcast-envelope.md`; `scope-drd/notes/FA4/h200/tp/pp-topology-pilot-plan.md`.)
...
- **Gflops scale quadratically with token count**: Halving patch size → 4× tokens → at least 4× Gflops (more due to quadratic attention). In Scope’s canonical 320×576 setting, `S=2160` tokens, so increasing spatial/temporal tokens quickly dominates both compute and comm surfaces (e.g., TP all-reduces on `[1,2160,5120]`). (See: `scope-drd/notes/FA4/h200/tp/feasibility.md` Section 1-2; `scope-drd/notes/FA4/h200/tp/explainers/04-tp-math.md`.)
