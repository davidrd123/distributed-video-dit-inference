...
*Behind the bitter lesson is a legion of engineers keeping GPUs running efficiently.*
...
[Figure: Table from 2004 ACM paper showing CPU FLOPS doubling time vs memory bandwidth doubling time, illustrating the growing compute-bandwidth gap]
...
[Figure: Factory analogy diagram showing that even though factory's size (FLOPS) doubled, if bandwidth can't keep up then performance isn't also going to double]
...
[Figure: Table showing FLOP counts for BERT by operator type - Tensor Contraction (matmuls) dominate at 99.8%, normalization and pointwise ops are 0.2%]
...
[Figure: Diagram showing a sequence of pointwise operators with data being sent back and forth to global memory between each operation]
...
[Figure: Diagram showing fused operations -- instead of sending data back to global memory between operations, all operations are performed in one go before writing back]
...
With the help of a fusing compiler like NVFuser, it's actually fairly easy to measure this ourselves!
...
[Figure: Three log-log plots showing runtime, FLOPS achieved, and memory bandwidth achieved as a function of compute intensity (repeat count). Runtime stays flat until ~64 multiplications then rises linearly. FLOPS start at 0.2 TF and rise linearly to peak ~9.75 TF. Memory bandwidth starts near peak ~1.5 TB/s and drops as compute intensity increases.]
...
As a result, we start off by achieving a measly 0.2 teraflops. As we double the compute intensity, this number grows linearly, until we get close to our peak of 9.75 teraflops [1]. Once we're close to our peak teraflops, we are considered to be "compute bound".
...
[Figure: Flamegraph profile of a single PyTorch addition operation, showing the vast majority of the execution trace is overhead (framework dispatch, Python interpreter, etc.) with only a tiny box representing actual computation]
...
[Figure: Timeline diagram showing GPU and CPU execution. When GPU operators are big enough, CPU runs ahead of GPU and overhead is hidden. When GPU operators are too small, GPU spends most of its time idle waiting for CPU.]
...
[Figure: PyTorch profiler trace showing lots of gaps on the GPU while it's waiting for CPU overhead]

*Lots of gaps on the GPU while it's waiting for CPU overhead*

[Figure: PyTorch profiler trace showing CPU running way ahead of GPU -- no gaps, fully utilized]
