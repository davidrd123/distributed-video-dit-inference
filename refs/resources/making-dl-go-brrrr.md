...
- **Profile first, optimize second**: In Scope TP v0, block profiling at 320×576 showed **decode 107ms (16.5%)** and **recompute_kv_cache 104ms (16.0%)** vs denoise 435ms (66.8%), i.e. decode+recompute is ~33% of measured GPU time — not TP-sharded and currently duplicated on worker ranks. (See: `scope-drd/notes/FA4/h200/tp/bringup-run-log.md` Run 10b.)
...
- **Use torch.compile / CUDA graphs for overhead**: Our TP=2 compile attempt was “overhead-bound” until we removed collective-induced graph breaks — **9.6 FPS with ~160 graph breaks** (Runs 8-9b) → **24.5 FPS** after functional collectives (Run 12b). Treat CUDAGraphs as a separate, higher-risk milestone on this stack. (See: `scope-drd/notes/FA4/h200/tp/bringup-run-log.md` Runs 8-12b; `scope-drd/notes/FA4/h200/tp/streamdiffusion-v2-analysis-opus.md`.)
...
- **Prove overlap with a metric, not vibes**: PP bringup uses `period ≈ max(Stage0, Stage1)` and `OverlapScore = median(hidden/min(stage0,stage1))` with pass gate ≥ 0.30 to decide if decode/IO is actually hidden or if you’re still overhead/serialization bound. (See: `scope-drd/notes/FA4/h200/tp/pp0-bringup-runbook.md` Phase 2; `scope-drd/notes/FA4/h200/tp/pp-control-plane-pseudocode.md` Instrumentation.)
- **Watch the bandwidth-compute gap across GPU generations**: On our targets, single-GPU FPS tracks HBM bandwidth (H200 ~4.8 TB/s ≈ 20 FPS vs B200/B300 ~8 TB/s ≈ 33–34 FPS), reinforcing that many kernels are memory-bound and extra compute often doesn’t help. (See: `scope-drd/notes/FA4/h200/tp/streamdiffusion-v2-analysis-opus.md` Hardware Profile.)
